...
class GaussianModel:
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(center, scaling, scaling_modifier, rotation):
            normal_scale = torch.ones_like(scaling[:, :1])
            surfel_scale = torch.cat(
                [scaling * scaling_modifier, normal_scale], dim=-1
            )
            RS = build_scaling_rotation(surfel_scale, rotation).permute(0, 2, 1)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        #rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda") #for test
    ...
    def gaussian_deformation(self, t, ch_num=9, basis_num=17):
        """
        Applies linear combination of learnable Gaussian basis functions to model the surface deformation.

        Args:
            t (torch.Tensor): The input tensor.
            ch_num (int): Deformation channels: 3 position, 4 rotation, and 2 surfel scale.
    ...
    def deformation(self, xyz: torch.Tensor, scales: torch.Tensor, rotations: torch.Tensor, time: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Args:
            ...
            scales (torch.Tensor): The two tangent-plane scales. (shape: [N, 2])
        ...
        """
        if getattr(self.args, "disable_deformation", False):
