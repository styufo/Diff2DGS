...
# 读取流程：
# 加载图像和深度图：
# 读取图像文件和对应的深度图文件，并将它们转换为适合处理的格式（如张量）。
# 加载相机参数：
# 读取相机的内参和外参，并将它们转换为适合计算的矩阵形式。
# 数据预处理：
# 对图像和深度图进行预处理，如归一化、裁剪、填充等。
# 数据增强：
# 对图像和深度图进行数据增强，如旋转、缩放、翻转等，以增加数据的多样性。
# 返回处理后的数据：
# 将处理后的图像、深度图和相机参数打包成一个数据结构，并返回用于训练模型。

def generate_se3_matrix(translation, rotation_rad):
    """
    生成一个 SE3 矩阵，用于表示三维空间中的刚体变换。
    它接受平移向量和旋转角度（以弧度为单位）作为输入。
    生成绕 x、y 和 z 轴的旋转矩阵，并将它们组合成一个总的旋转矩阵。
    最后，将旋转矩阵和平移向量组合成一个 4x4 的 SE(3) 矩阵。
    """
...
class EndoNeRF_Dataset(object):
    ...
    def __init__(
        ...
        self.dataset_type = self._detect_dataset_type(dataset_type)
    ...
    def _detect_dataset_type(self, requested):
        if requested != "auto":
            return requested
        path_hint = os.path.abspath(self.root_dir).lower()
        image_names = os.listdir(os.path.join(self.root_dir, "images"))
        if "stereomis" in path_hint or "stereo_" in path_hint:
            return "stereomis"
        if any(".color." in name.lower() for name in image_names):
            return "stereomis"
        return "endonerf"
    ...
    def load_meta(self):
        ...
        if self.dataset_type == "stereomis":
            ...
            H, W, focal = poses[0, :, -1]
            self.img_wh = (
                int(round(W / self.downsample)),
                int(round(H / self.downsample)),
            )
            focal = focal / self.downsample
            self.focal = (focal, focal)
            self.K = np.array([[focal, 0, self.img_wh[0] / 2],
                                        [0, focal, self.img_wh[1] / 2],
                                        [0, 0, 1]]).astype(np.float32)
            poses = np.concatenate([poses[..., :1], -poses[..., 1:2], -poses[..., 2:3], poses[..., 3:4]], -1)
        ...
        def agg_fn(filetype):
            ...
            for extension in ("png", "jpg", "jpeg", "bmp", "tif", "tiff"):
                files.extend(glob.glob(os.path.join(self.root_dir, filetype, f"*.{extension}")))
                files.extend(glob.glob(os.path.join(self.root_dir, filetype, f"*.{extension.upper()}")))
        ...
        print("number of masks:", len(self.masks_paths))
        print("number of images:", len(self.image_paths))
        print("number of depths:", len(self.depth_paths))
        print("number of poses:", poses.shape[0])
    ...
    def filling_pts_colors(self, filling_mask, ref_depth, ref_image):
        ...
        - ref_image: 参考图像
        返回：
        - refined_rgb: 填充后的 RGB 图像
        - refined_depth: 填充后的深度图
        """
         # bool
