...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background,\
    no_fine, benchmark_fps=False, reconstruct=False, crop_size=0):
    ...
    height, width = view.image_height, view.image_width
    intrinsics = getattr(view, "K", None)
    if intrinsics is not None:
        ...
        focal_x = fov2focal(view.FoVx, width)
        focal_y = fov2focal(view.FoVy, height)
        principal_x, principal_y = width / 2, height / 2
    camera_parameters = (
        focal_x,
        focal_y,
        principal_x,
    ...
    if reconstruct:
        ...
        reconstruct_point_cloud(
            ...
            render_depths,
            camera_parameters,
            os.path.join(model_path, "reconstruct", name),
            crop_size,
        )

def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool, reconstruct_train: bool, reconstruct_test: bool, reconstruct_video: bool, benchmark_fps: bool):
    with torch.no_grad():
        ...
        if not skip_video:
            render_set(dataset.model_path,"video",scene.loaded_iter, scene.getVideoCameras(),gaussians,pipeline,background, False, benchmark_fps=benchmark_fps, reconstruct=reconstruct_video, crop_size=20)

def reconstruct_point_cloud(images, depths, camera_parameters, output_frame_folder, crop_left_size=0):
    import cv2
    import copy
    os.makedirs(output_frame_folder, exist_ok=True)
    frames = np.arange(len(images))
    # frames = [0]
    focal_x, focal_y, principal_x, principal_y, width, height = camera_parameters
    if crop_left_size > 0:
        width = width - crop_left_size
        height = height - crop_left_size//2
        principal_x = principal_x - crop_left_size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--benchmark_fps", action="store_true")
